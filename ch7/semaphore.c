 ...
 *      ChangeLog:  1, Release initial version on "2023年04月06日 11时19分41秒"
 *                 
 ********************************************************************************/
...
int main(int argc, char **argv)
{
	...
	if( (pid = fork()) < 0 )
	{
		printf("Fock() failure: %s!", strerror(errno));
		return -2;
	}
	else if( 0 == pid )
	{
...
int semaphore_v(int semid)
{
	...
	if( semop(semid, &_sembuf, 1) < 0 )
	{
		printf("Semop V poerator failure: %s!\n", strerror(errno));
		return -1;
	}
